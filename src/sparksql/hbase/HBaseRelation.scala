...
case class HBaseRelation(@transient val hbaseProps: Map[String, String])(@transient val sqlContext: SQLContext) extends BaseRelation with Serializable with TableScan {
  ...
  val clientPort = hbaseProps.getOrElse("zookeeper_clientPort", sys.error("not valid zookeeper clientPort"))
  val quorum = hbaseProps.getOrElse("zookeeper_quorum", sys.error("not valid zookeeper quorum"))
